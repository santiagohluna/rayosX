...
then
    echo
    read -p 'Ingrese el ID de la observación a reducir: ' -e obsid
else
    obsid=$1
fi
...
evselect table=PN.fits withrateset=Y rateset=ratesPN.fits maketimecolumn=Y timebinsize=100 makeratecolumn=Y expression='#XMMEA_EP && (PI>10000&&PI<12000) && (PATTERN==0)' >> $LOG_FILE 2>&1
evselect table=M1.fits withrateset=Y rateset=ratesM1.fits maketimecolumn=Y timebinsize=100 makeratecolumn=Y expression='#XMMEA_EM && (PI>10000) && (PATTERN==0)' >> $LOG_FILE 2>&1
evselect table=M2.fits withrateset=Y rateset=ratesM2.fits maketimecolumn=Y timebinsize=100 makeratecolumn=Y expression='#XMMEA_EM && (PI>10000) && (PATTERN==0)' >> $LOG_FILE 2>&1
...
tabgtigen table=ratesPN.fits expression='RATE<=0.4' gtiset=PNgti.fits >> $LOG_FILE 2>&1
tabgtigen table=ratesM1.fits expression='RATE<=0.35' gtiset=M1gti.fits >> $LOG_FILE 2>&1
tabgtigen table=ratesM2.fits expression='RATE<=0.35' gtiset=M2gti.fits >> $LOG_FILE 2>&1
...
evselect table=PN.fits withfilteredset=Y filteredset=PNclean.fits destruct=Y keepfilteroutput=T expression='#XMMEA_EP && gti(PNgti.fits,TIME) && (PI>150)' >> $LOG_FILE 2>&1
evselect table=M1.fits withfilteredset=Y filteredset=M1clean.fits destruct=Y keepfilteroutput=T expression='#XMMEA_EM && gti(M1gti.fits,TIME) && (PI>150)' >> $LOG_FILE 2>&1
evselect table=M2.fits withfilteredset=Y filteredset=M2clean.fits destruct=Y keepfilteroutput=T expression='#XMMEA_EM && gti(M2gti.fits,TIME) && (PI>150)' >> $LOG_FILE 2>&1
